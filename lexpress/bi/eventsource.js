...
function EventSource(url, eventSourceInitDict) {
  ...
  function onConnectionClosed() {
    ...
    if (timeout_interval) clearInterval(timeout_interval);
  ...
  function connect() {
    ...
    console.error('timeout retry connect', url)
    ...
    if (eventSourceInitDict && eventSourceInitDict.timeout) {
      var timeout = eventSourceInitDict.timeout;
    }
    var lastMessageTime
    if (timeout) {
      timeout_interval = setInterval(function () {
        if (Date.now() - lastMessageTime > timeout) {
          onConnectionClosed()
          _emit('timeout', new Event('timeout'));
